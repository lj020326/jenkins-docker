...
######
##
## ref: https://github.com/felleslosninger/eid-jenkins-docker/blob/master/docker/jenkins/entrypoint.sh
######
...
createCredentials() {
    ...
    while IFS= read -r -d '' sshKeyFile; do
        createSshKeyCredential "${sshKeyFile}"
    done < <(find /run/secrets -type f -name "ssh.*" -print0 || true)

    while IFS= read -r -d '' f; do
        createUserPassCredential "$(basename "${f//_username}")"
    done < <(find /run/secrets -type f -name "docker_registry_*_username" -print0 || true)
